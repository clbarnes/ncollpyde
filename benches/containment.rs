...
pub fn mesh_contains_point_rays(mesh: &TriMesh, point: &Vector, ray_directions: &[Vector]) -> bool {
...
pub fn mesh_contains_point_oriented(mesh: &TriMesh, point: &Vector) -> bool {
    mesh.pseudo_normals().expect("Mesh orientation not checked");
    mesh.contains_local_point(*point)
}

pub fn mesh_contains_point_oriented_aabb(mesh: &TriMesh, point: &Vector) -> bool {
    mesh.pseudo_normals().expect("Mesh orientation not checked");
...
fn points_for_mesh(m: &TriMesh, n: usize) -> Vec<Vector> {
    ...
    repeat_with(|| {
        Vector::new(
            rng.random::<f64>() * new_range.x + mins.x,
            rng.random::<f64>() * new_range.y + mins.y,
            rng.random::<f64>() * new_range.z + mins.z,
        )
    })
    .take(n)
    .collect()
...
fn rays_for_mesh(mesh: &TriMesh, n: usize) -> Vec<Vector> {
